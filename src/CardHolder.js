    ...
    render() {
        ...
        <>
            ...
            <div className="CardHolderContainer" style={{bottom: "0px", alignItems: "flex-end"}}>
                {this.props.yourCards.map((val, index) => {return <Card position={"_P_" + index} playable={playablePlayerCards.includes(val)} clickable={this.props.turn === 0 && !this.props.animating} makeMove={this.props.makeMove} key={val + "_" + index} cardtype={val} ></Card>})}
            </div>
        </>)
    }
}
